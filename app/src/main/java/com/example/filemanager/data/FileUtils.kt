...
object FileUtils {
    ...
    fun openFile(context: android.content.Context, file: com.example.filemanager.data.FileModel) {
        try {
            val uri = androidx.core.content.FileProvider.getUriForFile(context, "${context.packageName}.provider", File(file.path))
            val mimeType = if (file.type == com.example.filemanager.data.FileType.APK) "application/vnd.android.package-archive" else file.mimeType
